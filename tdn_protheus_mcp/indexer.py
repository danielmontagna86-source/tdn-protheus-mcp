...
CHUNK_SIZE = 1800
CHUNK_OVERLAP = 200
_PARAMETER = re.compile(r"\bMV_[A-Z0-9_]{3,}\b", re.IGNORECASE)
_TABLE = re.compile(
...
def _metadata(record: dict[str, Any]) -> dict[str, list[str]]:
    ...
    source = f"{title}\n{text}"
    routines = _existing(record, "routines") | {
        item.upper() for item in _ROUTINE.findall(source)
    }
    tables = _existing(record, "tables") | {
        item.upper() for item in _TABLE.findall(source)
    }
    parameters = _existing(record, "parameters") | {
        item.upper() for item in _PARAMETER.findall(source)
    }
    entry_points = _existing(record, "entry_points") | {
        item.upper() for item in _EXPLICIT_ENTRY.findall(source)
    }
    if "PONTO DE ENTRADA" in title.upper():
        entry_points |= {
            item
            for item in {token.upper() for token in _ROUTINE.findall(title)}
            if not item.startswith(_PROGRAM_PREFIXES)
        }
    modules = _existing(record, "modules") | {
        name
        for name, pattern in _MODULE_PATTERNS.items()
...
class SnapshotIndexer:
    ...
    def _index_path(self, root_id: str) -> Path:
        ...
        return self._policy.require_path(
            self._policy.cache_root / normalized / "index.sqlite3"
        )
    ...
    def _create_schema(
        ...
        connection.executescript(
            ...
            CREATE INDEX idx_chunk_tags_kind_value
                ON chunk_tags(kind, value, chunk_id);
            """
        )
            ...
            try:
                ...
                for record in self._repository.active_pages(normalized):
                    ...
                    for chunk_index, content in enumerate(
                        _chunks(str(record.get("text", "")))
                    ):
                        chunk_id = f"{page_id}:{chunk_index}"
                        connection.execute(
                            """
                            INSERT INTO chunks(
                                chunk_id, root_id, page_id, title, source_url,
                                version_number, collected_at, modules_json,
                                tables_json, parameters_json, routines_json,
                            ...
                            (
                                ...
                                json.dumps(metadata["modules"], ensure_ascii=False),
                                json.dumps(metadata["tables"], ensure_ascii=False),
                                json.dumps(metadata["parameters"], ensure_ascii=False),
                                json.dumps(metadata["routines"], ensure_ascii=False),
                                json.dumps(
                        ...
                        for kind in (
                            ...
                            connection.executemany(
                                ...
                                [
                                    (chunk_id, singular, value)
                                    for value in metadata[kind]
                                ],
                            )
                        chunk_count += 1
                connection.commit()
