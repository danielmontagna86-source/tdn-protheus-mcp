...
_ALLOWED_FIELDS = {"cache_root", "allowed_root_ids", "max_results", "max_chars", "offline", "allow_mutations"}
...
def load_config(path: str | Path) -> McpConfig:
    ...
    if data.get("offline", True) is not True or data.get("allow_mutations", False) is not False:
        raise ConfigError("CONFIG_READ_ONLY_REQUIRED", "o MCP é somente leitura; use offline=true e allow_mutations=false")

    cache_root = Path(_required_string(data, "cache_root")).expanduser().resolve()
    root_ids = data.get("allowed_root_ids")
    if (
        not isinstance(root_ids, list) or not root_ids
        or any(not isinstance(item, (str, int)) or not str(item).strip().isdigit() or int(str(item).strip()) <= 0 for item in root_ids)
    ):
        raise ConfigError("CONFIG_INVALID_ROOTS", "'allowed_root_ids' deve ser uma lista não vazia de identificadores")
    return McpConfig(
        cache_root=cache_root,
        allowed_root_ids=frozenset(str(int(str(item).strip())) for item in root_ids),
