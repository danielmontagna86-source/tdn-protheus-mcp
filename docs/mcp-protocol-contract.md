...
Erros recusados incluem `POLICY_ROOT_NOT_ALLOWED`, `POLICY_PATH_OUTSIDE_CACHE`, `POLICY_INDEX_NOT_FOUND`, `POLICY_PAGE_NOT_ALLOWED`, `POLICY_LIMIT_EXCEEDED`, `POLICY_OFFLINE`, `POLICY_MUTATIONS_DISABLED`, `POLICY_CONFIRMATION_REQUIRED`, `POLICY_REFRESH_CANCELLED`, `POLICY_REFRESH_TIMEOUT`, `POLICY_REFRESH_BUSY`, `POLICY_SNAPSHOT_NOT_FOUND` e erros `CONFIG_*`. Uma atualização que não consiga consultar o endpoint configurado retorna `UPSTREAM_TDN_REQUEST_FAILED`; uma resposta incompatível, incluindo paginação fora da API configurada, retorna `UPSTREAM_TDN_INVALID_RESPONSE`. A resposta não inclui caminhos fora de `cache_root`, HTML bruto, tokens ou credenciais.

Após uma atualização bem-sucedida, o cache mantém apenas a geração atual e a imediatamente anterior. O manifesto aponta atomicamente para a geração ativa; leituras normais não fazem rede. O índice FTS é removido para ser reconstruído explicitamente com `index`.
